...
description: "Build or revise publication-ready scientific figures—including motivation/method diagrams and data-bearing result panels—from the user's narrative and data/code: choose an appropriate editable vector source (often Python/Matplotlib for dense methods or curves), add optional non-quantitative raster illustrations, and deliver an organized SVG/PNG/PDF package with assets, provenance, and QA. Do not invent or alter quantitative results."
metadata:
...
Every completed figure must be handed off as a versioned, self-contained package, not as a lone PNG. The canonical composition source may be code-native (for example, Python/Matplotlib) or a hand-authored SVG; select it from the figure's needs rather than forcing one composer:

- Keep text, mathematical notation, axes, ticks, legends, arrows, borders, guides, and data-bearing geometry as live vector text/shapes/paths. Do not bake the whole figure, labels, or chart into one raster generated by ImageGen.
- Prefer Python/Matplotlib as the canonical source for dense multi-band method figures, aligned trajectories, charts, or any composition with code-rendered curves. Let Python own the full canvas, geometry, labels, and asset placement, then export SVG/PDF/PNG. Set `svg.fonttype = "none"` and PDF font type 42 so the SVG text stays editable and PDF fonts stay embedded.
- Use a hand-authored SVG framework for a simple diagram only when it is the clearer, more maintainable source or the user requests it. If the user asks for Python/code-native drawing, do not hand-write SVG.
...
- Deliver the current SVG, PNG, and PDF rendered from the same source, plus that canonical source (or its exact repository path and revision), all accepted assets, prompt/provenance records, a manifest, QA notes, previews, and superseded versions in clearly named subdirectories. Read [references/deliverable-package.md](references/deliverable-package.md) for the required tree and naming convention.

Before calling a figure complete, inspect the exported SVG in an editor or text/XML path and verify that labels remain selectable/editable and raster assets remain independently replaceable. A PNG-only or ImageGen-flattened “SVG” is a packaging blocker unless the user explicitly requested a raster-only deliverable.

## Governing principle

Separate semantic geometry from visual generation:

- The selected vector source—normally Python/Matplotlib for dense scientific compositions, otherwise SVG or another appropriate vector system—owns the canvas, panel geometry, alignment, arrows, guides, axes, labels, typography, legends, and any exact curves or tables.
...
The experimental branch still uses the selected code/vector framework, revision freeze, export, and QA rules below. The key invariant is that a later palette or layout change must not alter the underlying numerical geometry.

## 3. Lock a storyboard and vector framework

Choose the canonical composer before building the framework:

- Use Python/Matplotlib when the figure contains data curves, repeated aligned elements, many cards/labels, a multi-band method layout, or a supplied reference whose geometry should be reproduced precisely. Draw the whole composition in Python; do not generate a separate hand-written SVG skeleton first.
- Use direct SVG for a small, mostly geometric diagram when its XML remains easier to maintain than code.
- In either case, create a lightweight framework/preview that renders meaningfully with placeholder rectangles before generating raster assets.
...
2. Establish named layers/groups or clearly separated drawing functions/artists for background, raster placeholders, structural lines, charts/signals, and text. Keep shared colors, line widths, font sizes, and spacing in one token block so local revisions do not drift geometry.
3. Use Times New Roman (with an explicit fallback) consistently for all figure text when that is the paper style. Keep text as live SVG/Matplotlib text, never baked into a generated image. For Matplotlib, verify `svg.fonttype="none"`, configure math text from the same font family where possible, and audit the exported fonts for fallback.
4. Put exact axes, ticks, legends, arrows, dashed guides, clip boundaries, and curves in the canonical vector source. Use shared x-coordinates for aligned time elements rather than eyeballing them.
...
Render and inspect this framework/preview before generating assets. Fix confusing hierarchy or spacing at this stage; do not try to solve a semantic layout problem with a prettier raster.
...
Place generated files through the chosen vector source using explicit boxes and deterministic crop/fit rules. In direct SVG, use portable references and `preserveAspectRatio`; in Matplotlib, use bounded image artists/axes and explicit clip paths or extents. A delivered standalone SVG should inline sanitized local image data (or be accompanied by a clearly named portable asset package). Draw frame borders, separators, dashed correspondence lines, axes, labels, and legends after raster layers so they remain crisp and readable.
...
Keep raster assets out of the text/axis layer. If a generated asset changes, replace only its image reference/path or bounded artist; do not ask ImageGen to regenerate the entire figure and thereby alter unrelated pixels.

## 6. Revision protocol: freeze everything else

When the user asks for a local correction, first define the authorized region and the invariant region. Create a new versioned filename, make the smallest possible canonical-source or asset change, and leave all unmentioned coordinates, wording, colors, and raster crops untouched. Typical local fixes include:

- a subject cutout: regenerate or replace only that cutout, then fit it with safe left/right margins;
- a label artifact: update only that label artist/text object and its immediate bounding area;
...
- **G2 — framework:** the code/vector framework renders; dimensions, fonts, axes, alignment, and geometry pass inspection before raster assets are added. If SVG is exported, it also parses with editable text and unique IDs.
...
- **G6 — package:** PASS only when `final/` contains the current SVG + PNG + PDF, the canonical source/path is recorded, every local asset and manifest path resolves, `assets/`/`prompts/`/`previews/`/`archive/` are classified, and authorized data-to-geometry provenance is reproducible; any missing export, unresolved reference, placeholder, or flattened-only source is a BLOCKER. Manuscript integration is included when the user requests it.
...
├── previews/    # lightweight renders of the canonical framework/source
├── source/      # optional vendored code/SVG; otherwise manifest exact repo path
├── frameworks/  # optional: hand-authored SVG skeletons only
